...
export function generarPDF(proyecto, config, pdfConfig, lineas, totales) {
  ...
  if (pdfConfig?.mostrarCliente && proyecto.cliente) {
    const yClienteBase = y - (proyecto.notas ? (doc.splitTextToSize(proyecto.notas, colW - 4).length * 4 + 5) : 5)
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(7)
    doc.setTextColor(...COLOR_MUTED)
    doc.text('CLIENTE', margen + colW, yClienteBase)
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(11)
    doc.setTextColor(...COLOR_DARK)
    doc.text(proyecto.cliente, margen + colW, yClienteBase + 5)
  }
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: COLOR_DARK, textColor: COLOR_WHITE, fontStyle: 'bold', fontSize: 7.5 },
